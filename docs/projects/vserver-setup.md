...
while IFS= read -r line; do
  echo "$line" | ./scripts/vault_string.sh -e -v product
done < secrets.txt
```

**2. Quick Decrypt for Testing:**
```bash
# Temporarily decrypt to verify values
./scripts/vault_string.sh -d -i group_vars/all/users.yml | grep password
```

**3. Batch Encryption:**
```bash
# Encrypt all passwords in a file
for var in password1 password2 password3; do
  echo "Enter $var:"
  ./scripts/vault_string.sh -e -v product > "${var}_encrypted.txt"
done
```

### Ansible Vault Tips

**1. Use Vault IDs for Environments:**
```bash
# Different vaults for different environments
ansible-vault encrypt_string --encrypt-vault-id dev "dev-password"
ansible-vault encrypt_string --encrypt-vault-id prod "prod-password"
```

**2. View Without Decrypting:**
```bash
# See structure without decrypting
ansible-vault view group_vars/all/users.yml
```

**3. Rekey Vault Files:**
```bash
# Change vault password
ansible-vault rekey group_vars/all/users.yml
```

**4. Validate Before Committing:**
```bash
# Test that vault files are valid
ansible-vault decrypt --check group_vars/all/users.yml
```
